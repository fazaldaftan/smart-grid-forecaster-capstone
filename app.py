...
# 2. HELPER FUNCTIONS (Moved to top so Sidebar can use them instantly)
...
with st.sidebar:
    ...
    # Render Globe dynamically based on selected location
...
if st.session_state.get('fetch_data', False) and feature_cols is not None:
...
if st.session_state.get('app_ready', False):
    ...
    # --- UPDATED TAB 3: FINANCIAL ARBITRAGE + CARBON ---
    with tab3:
        # Math for Financials & Carbon
